...
最後に\ref{sec:observation_uncertainty}項では，前項までの議論が暗黙に仮定していた「観測データは真の行動を正確に反映している」という前提を問い直し，GPSやBLEなどのセンサデータに含まれる観測誤差のもとでの推定手法——経路観測モデル，構造推定，量子化と観測多様体——を紹介する．
...
しかし実際の交通行動データ，特にGPSやBLE（Bluetooth Low Energy）などのセンサデータに基づく観測では，観測誤差が不可避的に存在する．
...
本節では，この観測の不確実性に正面から向き合い，観測モデルと行動モデルを統合的に推定する枠組みを解説する．
...
特に歩行者ネットワークのように密なネットワークでは経路候補が多く，観測誤差に対して頑健な経路同定が困難になるため，バイアスはより深刻になる．

\subsubsection{経路観測モデルとリンク固有分散}\label{sssec:route_measurement}

Oyama and Hato (2018) \cite{Oyama2018gpseng} は，経路同定とパラメータ推定を分離せず，\textbf{観測モデル}と\textbf{行動モデル}を統合的に扱う枠組みを提案した．

その基本的な着想は，GPSの位置情報 $\hat{\bm{m}}$ と真の経路 $r$ の関係を確率的に記述する\textbf{経路観測モデル}（route measurement model）を導入することである．
リンク $a$ 上の位置 $x$ で観測されたGPS座標 $\hat{x}$ の観測確率を，リンク固有の分散 $\sigma_a^2$ を持つ正規分布でモデル化する．
...
従来の観測モデルではネットワーク全体で一定の分散 $\sigma$ を仮定していたが，実際にはアーケード街や高層ビル近辺では分散が大きく，見通しの良い道路では小さいといった\textbf{空間的な異質性}がある．
リンク固有の分散 $\sigma_a$ を導入することで，この異質性を明示的にモデルに組み込むことが可能になる．

観測モデルと行動モデルを結合した\textbf{リンク尤度}は，ベイズの定理により以下のように書ける．
...
\end{equation}
ここで $p(\hat{\bm{m}}^t \mid a_t; \sigma_{a_t})$ は観測モデル，$p(a_t \mid a_{t-1}; \theta)$ は行動モデル（RLモデルの遷移確率）である．
この式は，各時点で「観測データがどの程度そのリンクと整合的か」（観測尤度）と「行動モデルに基づいてそのリンクがどの程度選ばれやすいか」（事前確率）の積によって，リンクの事後確率を計算する構造になっている．

\subsubsection{構造推定：観測モデルと行動モデルの不動点}\label{sssec:structural_estimation}

上述のリンク尤度は観測パラメータ $\sigma$ と行動パラメータ $\theta$ の双方に依存するため，両者を同時に推定する必要がある．
Oyama and Hato (2018) \cite{Oyama2018gpseng} はこの問題を，観測モデルと行動モデルの\textbf{不動点}を探索する\textbf{構造推定}（structural estimation）として定式化した．

\begin{enumerate}
  \item \textbf{初期化:} 行動パラメータの初期値 $\theta^{(0)}$（例：無情報事前分布 $\theta^{(0)} = \bm{0}$）を設定する．
  \item \textbf{観測モデルの推定:} 行動パラメータ $\theta^{(h)}$ を固定し，観測モデルの分散 $\sigma$ を推定する．同時に推定された経路 $r^{(h)}$ を得る．
  \item \textbf{行動モデルの推定:} 推定された経路 $r^{(h)}$ を用いて行動パラメータ $\theta^{(h+1)}$ を最尤推定する．
  \item \textbf{収束判定:} $|\theta^{(h+1)} - \theta^{(h)}| < \xi$ であれば終了．そうでなければStep 2に戻る．
\end{enumerate}

この反復過程は，情報幾何学における多様体学習のEM（em, expectation-maximization）アルゴリズム \cite{Amari1995} と類似の構造を持つ．
Step 2は「現在の行動モデル（事前分布）のもとで観測データから経路を推定する」操作（E-step），Step 3は「推定された経路のもとで行動パラメータを更新する」操作（M-step）に対応する．

不動点では $\theta^{(h)} = \theta^{(h+1)}$ が成立し，観測モデルが推定に用いる事前情報（行動モデル）と，推定結果から得られる行動パラメータが\textbf{整合的}（self-consistent）になる．

\subsubsection{量子化と観測多様体}\label{sssec:quantization}
前項の構造推定は，各時点で「最も尤もらしい1本のリンク」を逐次的に同定する枠組みであった．
しかし，屋内のBLEビーコン観測のように観測点密度が低くかつ観測分散が大きい状況では上記の手法に基づいて算出される最尤のリンクですら信頼性が低く，逐次的な同定の誤りが後続の推定に波及するという問題が残る．
...
これは本質的にはOyama and Hato (2018) \cite{Oyama2018gpseng} の構造推定の一般化であり，経路同定を完全に回避したことで一つの目的関数\eqref{eq:generalized_ll}を直接最大化することが可能になった．


\subsubsection{潜在状態モデルとしての統一的理解}\label{sssec:latent_state}

以上で紹介した枠組みは，いずれも「真の経路は直接観測できない\textbf{潜在変数}（latent variable）であり，観測されるのは観測誤差を含んだセンサデータである」という視点に立つ．
Oyama and Hato (2018) \cite{Oyama2018gpseng}では経路列（クロネッカーデルタ）が潜在変数であったのに対し，Matsunaga and Hato (2026) \cite{matsunaga2026trc} では遷移尤度 $Q$ が潜在変数となっている．
% この構造は，\textbf{隠れマルコフモデル}（Hidden Markov Model; HMM）とも

% % \begin{description}
% %   \item[隠れ状態] 真の経路（リンクの系列）$r = [a_1, \ldots, a_T]$ないし状態遷移尤度 $Q$ 直接は観測できない．
