%% chapters/assignment/ch8_simulation.tex
%% 第8章　シミュレーション

%% NOTE: 第6章（進化ゲーム理論）・第7章（ピグー課金）は未執筆のため，
%%       assignment.tex 側で \setcounter{section}{7} を設定して
%%       この章が第8節として出力されるようにしている．
%%       第6・7章が追加された際は assignment.tex の setcounter 行を削除すること．

\section{シミュレーション}
\label{sec:ch8-simulation}

前節（\sref{sec:ch7-pigou}）まで，交通ネットワークの均衡・最適化・課金理論を
解析的・数値的手法で扱ってきた．
...
本節で用いる主要記号を表\ref{tab:ch8-notation}にまとめる．
...
\begin{figure}[tb]
  \centering
  \input{assets/assignment/fig_mfd_concept.tex}
...
\begin{figure}[tb]
  \centering
  \input{assets/assignment/fig_car_following.tex}
...
均衡配分理論（\sref{sec:ch3-ue}〜\sref{sec:ch5-so}）・進化ダイナミクス（\sref{sec:ch6-evogame}）・混雑課金（\sref{sec:ch7-pigou}）といった
...
\begin{enumerate}
  ...
  \item マクロ・ミクロの2手法は相補的であり，
        ...
        進化ダイナミクス（\sref{sec:ch6-evogame}）・
        混雑課金（\sref{sec:ch7-pigou}）といった理論的成果を
