...
条件\eqref{eq:wardrop-condition}を\textbf{相補性条件}（相補性条件および KKT 条件の一般形については補足D を参照）の形式に書き直すと，
...
\begin{figure}[hbtp]
  \centering
  \input{assets/assignment/fig_fw_step.tex}
...
\begin{itemize}
  \item \textbf{実装が単純}：各イテレーションは旅行時間計算 → All-or-Nothing
        配分 → 線探索 → 更新という明快な手順で構成される．
...
\begin{figure}[hbtp]
  \centering
  \input{assets/assignment/fig_braess_network.tex}
...
\begin{itemize}
  ...
  \item \textbf{SO との対比}：社会的に最適な配分で利用者を制御すれば，
        ...
        （Pigouvian 課金，\sref{sec:ch5-so}・\sref{sec:ch7-pigou}\cite{Pigou1920}）の根拠となる．
...
\begin{enumerate}
  ...
  \item \textbf{Braess のパラドックス}は，新規リンク追加後の Nash 均衡（UE）において
        ...
        UE と社会最適配分（SO）との乖離が
        混雑課金介入（\sref{sec:ch5-so}・\sref{sec:ch7-pigou}）の根拠となることを示唆する
        （\ssref{subsec:braess}）．
\end{enumerate}
