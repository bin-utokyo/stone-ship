%% chapters/assignment/ch7_pigou.tex
%% 第7章　ピグー課金と混雑課金の一般理論
...
\begin{tcolorbox}[
  ...
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item 課金なし（UE）では PMC のみを参照 → SMC > PMC により過剰交通量
...
通勤者の一般化費用は\textbf{$\alpha$-$\beta$-$\gamma$ 体系}\cite{Vickrey1969}：
...
\begin{enumerate}
  ...
  \item
    ...
    Vickrey（1969）によって示されている\cite{Vickrey1969}．
...
例えば「ピーク前（やや高）→ピーク（最高）→ピーク後（やや高）→夜間（無料）」という
3〜4段階の料金体系が実用的である．
シンガポールの ERP（Electronic Road Pricing）は
30分単位で料金を変動させる実装例として知られている\cite{Vickrey1969}．
...
\begin{itemize}
  \item ロンドン中心部課金（2003年導入）：年間約 1.5 億ポンドの収入
  \item ストックホルム中心部課金（2006年導入，2007年恒久化）：年間約 7 億クローナ
  \item シンガポール ERP：道路整備・公共交通補助の主要財源の一つ
\end{itemize}
...
\begin{enumerate}
  \item \textbf{第一配当}：混雑外部性の内部化 → 交通効率の改善（死荷重損失の解消）
  \item \textbf{第二配当}：租税の歪み（超過負担）の縮小 → 経済全体の効率改善
\end{enumerate}
...
\begin{tcolorbox}[
  ...
  title={\small まとめ：ピグー課金と混雑課金の一般理論},
  breakable, fonttitle=\bfseries\small
]
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item \textbf{外部性の内部化}（\ssref{subsec:pigou-externality}）：
          ピグー税 $\tau^* = V^* t'(V^*)$ で PMC を SMC に引き上げ，
          個人の私的最適化が社会最適と一致する
    \item \textbf{第一最善課金}（\ssref{subsec:first-best}）：
          全リンクに $\tau_a^* = x_a^{SO} t_a'(x_a^{SO})$ を設定することで，
          UE 解が SO 解に一致し，死荷重損失が解消される
    \item \textbf{第二最善課金}（\ssref{subsec:second-best}）：
          課金可能リンクが限定される場合，バイレベル計画問題を解く必要があり，
          単純な限界外部費用課金では不十分；迂回交通の考慮が不可欠
    \item \textbf{動的課金}（\ssref{subsec:vickrey-bottleneck}）：
          Vickrey ボトルネックモデルにおける台形型料金プロファイルにより
          行列（キュー）が消滅し，最大 50\% の社会損失削減が可能
    \item \textbf{収入の再分配}（\ssref{subsec:redistribution}）：
          適切な再分配によりパレート改善が可能；
          二重配当効果で経済政策としての根拠も生まれる
  \end{itemize}
\end{tcolorbox}
